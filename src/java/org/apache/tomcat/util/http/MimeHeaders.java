...
public class MimeHeaders {
     ...
     * 得到第i个Header的name。name和value都存储在Field中
     */
     ...
     * This may be used to iterate through all header fields.
     * 得到第i个Header的value。name和value都存储在Field中
     */
    ...
    */
    ...
    /**
     * 返回name唯一的Field的值。如果该name不存在，则返回null。如果该name存在多份，则抛异常
...
class NamesEnumerator implements Enumeration {
    ...
    private void findNext() {
        ...
        for(  ; pos< size; pos++ ) {//不断循环header
            ...
            //如果next为null，说明以前已经有重名的Header了，因此继续循环下一个位置的内容，如果next不是null,则说明以前没有出现过，则直接返回该值，因为该值就是第一次出现的Header
            if( next!=null ) {
...
class ValuesEnumerator implements Enumeration {
    int pos;//当前查询到第几个表头了
    int size;//headers中总共包含的表头个数
    MessageBytes next;//下一个名称为name的值
    MimeHeaders headers;//整体headers参数
    String name;//待查询的name值

    /**
     * 初始化信息，然后调用findNext方法，查找符合name的下一个Header
     * @param headers
     * @param name
     */
    ...
    /**
     * 从当前位置开始查询，一直到最后，如果存在name名称的Header，则将next赋值给他，说明下一个位置就是他。当前位置向后移动一位。
     */
    private void findNext() {
        next=null;
    ...
    /**
     * 根据next是否为空，可以得到是否还有一下个了。
     */
    public boolean hasMoreElements() {
        return next!=null;
    }
/**
 * 获取下一个元素之，即获取next值，然后返回。在返回前调用findNext方法，查找下一个是否存在。
 */
    public Object nextElement() {
