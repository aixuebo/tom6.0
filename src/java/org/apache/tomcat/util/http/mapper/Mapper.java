...
public final class Mapper {
     ...
     * 添加一个host，host包含name和host对象，以及host中包含的ContextList，即Context集合
     */
     ...
     *
     * @param name Virtual host name
     * 根据name移除某一个host，首先查看是否有该name对应的Host对象，如果存在，则移除该host。
     * 然后循环剩余的Host集合，找到Host对应的真正Object同名的(即同一个Host的其他所有别名，但是他们公用同一个)，继续移除。
     */
     ...
     * 为真是存在的host添加别名。 首先找到该Host对象。然后创建新的Host对象，将其中的属性用查询到的Host赋值。
     */
     ...
     * 移除别名Host，即仅仅移除指定名称的host，不用移除与别名相同对象的所有host
     */
 ...
 */
     ...
     * @param welcomeResources Welcome files defined for this context
     * @param resources Static resources of the context
     * 1.根据hostName找到该Host对象。
     * 2.计算path所包含的/数量，设置该Host对应的最长的Path分隔次数。因为同一个Host,可以有多少Path。
     * 3.通过该Host对象得到该Host中所有的Context集合。
     * 4.创建新的Context对象，设置path、context、welcomeResources、resources属性。将其添加到该host对应的Context集合中。
     */
     ...
     * @param hostName Virtual host name this context belongs to
     * @param path Context path
     * 1.通过hostName得到Host对象。
     * 2.循环该Host下所有的Context对象。并且删除指定path所对应的Context对象。
     * 3.如果删除成功，则重新计算Host对应的/最大分隔长度
     */
     ...
     * 返回左右的Host/path组成的数组，格式是//HOST/PATH
     ...
     * 为Context添加一个Wrapper,即为一个Web应用添加一个servlet
     */
    ...
    /**
     * 1.首先找到该Host对象。
     * 2.然后找到该host下所有的Context集合，在该集合下，找到与contextPath相同的Context对象。即找到了某个应用。
     * 3.调用addWrapper(context, path, wrapper, jspWildCard);方法，将wrapper添加到该context中
     */
    public void addWrapper(String hostName, String contextPath, String path,
     ...
     * 为默认Context添加wrapper对象，该对象所属path为路径
     */
     ...
     * 真正为Context(应用)添加一个wrapper(servlet)
     * 1.创建一个Wrapper对象，object属性就是wrapper，jspWildCard属性也是参数传递进来的。
     *   path和name属性需要规则判断：
     *a.如果path传递的是 /*结尾,则name就是刨除/*的字符串。如果该context中对应的wildcardWrappers中没有该name对应的wrapper，则将其添加到集合中。
     *b.如果path传递的是以*.开头的，则那么是刨除*.的字符串。将该wrapper添加到context的extensionWrappers中。
     *c.如果path为/,则name为"",设置wrapper为context的defaultWrapper。
     *d.否则name就是path，该wrapper为context的exactWrappers中添加。
     */
    protected void addWrapper(Context context, String path, Object wrapper,
                              boolean jspWildCard) {
     ...
     * @param path Wrapper mapping
     * 1.先找到host对象。
     * 2.通过contextPath找到对应的Context对象。
     * 3.调用removeWrapper(context, path);方法，即要删除context对象中符合path的wapper
     */
...
/**
 * 要删除context对象中符合path的wapper，该方法逻辑与添加逻辑相反
 * @param context
 * @param path
 */
    ...
    /**
     * 1.得到该Context对象下所有的Wrapper的path集合
     * 2.将所有集合用:分隔打印出来
     * @param host
     * @param context
     * @return
    ...
    /**
     * 得到该Context对象下所有的Wrapper的path集合
     * 1.通过host的name找到Host对象。
     * 2.通过Host对象找到该Host下所有的Context对象集合。
     * 3.通过context找到该Context集合中匹配的Context对象，
     * 4.得到该Context对象下所有的Wrapper的path集合
     * @param host
     * @param context
     * @return
     */
     ...
     * 比较name从start位置到end位置所对应的字符串与compareTo做比较。
     * name>compareTo 则返回1.否则返回-1.相等返回0
     */
     ...
     * duplicates.
     * 添加成功 返回true，添加失败，则返回false。失败的原因是老集合中已经存在了待添加的元素
     */
    private static final boolean insertMap
        (MapElement[] oldMap, MapElement[] newMap, MapElement newElement) {
        int pos = find(oldMap, newElement.name);
        if ((pos != -1) && (newElement.name.equals(oldMap[pos].name))) {//newElement已经在oldMap中存在了，则返回false，表示没有添加成功
            return false;
        }
        /**
         * 如果不存在，则将老的集合中数据添加到新集合中，并且排序添加。排序的原则是pos得到的位置已经是有顺序的了。
         */
    ...
    public static void main(String args[]) {
        ...
        System.out.println("size==>"+mapper.hosts.length);
