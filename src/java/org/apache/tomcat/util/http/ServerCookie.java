...
public class ServerCookie implements Serializable {
    ...
    private static final String tspecials2NoSlash = "()<>@,;:\\\"[]?={} \t";//该字符串与tspecials2相比较，少个/字符
 ...
 */
    public static boolean containsCTL(String value, int version) {
        ...
        for (int i = 0; i < len; i++) {
            char c = value.charAt(i);
            if (c < 0x20 || c >= 0x7f) {//0x20=32 0x7f=127  因为32-127之间都是字母和有效字符，32之前是控制字符
                if (c == 0x09)//0x09 = 9 为tab字符
    ...
    /**
     * @deprecated - Not used
     * 如果name中包含,; ,或者name为以下字符串，则返回false
     */
     ...
     * 根据不同版本，返回请求头。该类实现是无论什么版本，请求头都是Set-Cookie，但是按照协议来说应该是Set-Cookie2.不知道为什么
     */
    ...
    /**
     * value是否已经用双引号包含了，即value的第一位和最后一位是否是双信号
     * true表示已经被双引号包围。false表示没有被双引号包围
     * @param value
     * @return
     */
    ...
    /**
     * 1.如果value为空，则向StrigBuffer中添加双引号""
     * 2.如果value包含控制字符，则抛异常，提示cookie中不允许有控制字符，如果需要，则要进行base64编码
     * 3.已经被双引号包围,则将value去除双引号，然后在用双引号添加到buf中
     * @param version
     * @param buf
     * @param value
     * @param literals
