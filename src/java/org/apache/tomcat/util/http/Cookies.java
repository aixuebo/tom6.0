...
/**
 * 一个cookies集合，可以再次使用，并且调节服务端性能的基础
...
public final class Cookies { // extends MultiMap {
    ...
    // expected average number of cookies per request 期望每次请求的平均cookie是4个
    public static final int INITIAL_SIZE=4; 
    ServerCookie scookies[]=new ServerCookie[INITIAL_SIZE];//初始化数组，存放cookie用
    int cookieCount=0;//目前已经存储了多少个cookie
    boolean unprocessed=true;//是否解析了header true表示没有解析 false表示已经解析
     ...
     * being quoted.
     * 如果是true，表示cookie的值可以包含=号，并且还等号没有引号
     */
    ...
    */
    ...
    /**
     * 构造一个新的cookie集合，该集合从参数MimeHeaders中提取cookie信息
    ...
    public ServerCookie getCookie( int idx ) {
        if( unprocessed ) {//没有解析，则先进行解析，然后返回第几个cookie对象
    ...
    /**
     * 解析请求头，设置参数unprocessed为false,表示解析过了，
     * @return
     */
     ...
     */
    public ServerCookie addCookie() {//如果cookie的数量已经超过了现有的， 则重新创建一个2倍大小的cookie池。
        if( cookieCount >= scookies.length  ) {//目前存储的cookie已经大于缓存的大小，则扩展
     ...
     * 解析请求头中所有的Cookie请求头，每解析一个，就调用processCookieHeader方法
     */
    public  void processCookies( MimeHeaders headers ) {
        ...
        while( pos>=0 ) {
            ...
            pos=headers.findHeader( "Cookie", pos );//找到Cookie标签。
            // no more cookie headers headers
            if( pos<0 ) break;//如果没有，则退出循环。

            MessageBytes cookieValue=headers.getValue( pos );//如果有，则获取该位置的值
    ...
    /**
     * 判断字符串s与字节数组从start到end之间的内容是否相同
     */
