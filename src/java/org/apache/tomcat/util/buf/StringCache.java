...
public class StringCache {
   ...
   /**
     ...
     * Access count.
     */
    protected static int accessCount = 0;//访问缓存的次数
    

    /**
     * Hit count.
     */
    protected static int hitCount = 0;//击中缓存的次数
    ...
    public static String toString(ByteChunk bc) {
        ...
        // still training
        if (bcCache == null) {
            String value = bc.toStringInternal();//返回字符串
            ...
            accessCount++;//访问缓存的次数增加
            // Find the corresponding String
            String result = find(bc);
            if (result == null) {//如果没有查找到，则调用string方法
     ...
     * 比较ByteChunk中从start开始，到end之间的字节，与compareTo比较。
     * 1.如果先比较两者长度len相同时，循环每一个字节，如果name中每一个字节都比compareTo大，则返回1.相反则返回-1。相等则返回0.
     * 2.如果两者不相同，则先按照相同比较，如果比较结果依然是0，则判断谁更长。
     *   如果name更长，则返回1.如果compareTo长，则返回-1
     *   
     *  总结：name比compareTo大，则返回1.相反比返回-1.相等则返回0
     */
     ...
     * 在缓存中查找，首先找到最接近的值。
     * 然后让最接近的值与参数name比较，从而找到原来缓存的字符串。
     */
     ...
     * given array.
     * 找不到则返回-1，找到了，则返回缓存数组的小标(返回结果是等于name的或者接近name的下标)
     * 
     */
    protected static final int findClosest(ByteChunk name, ByteEntry[] array, int len) {
        ...
        /**
         * b=-1，表示len-1=-1，即len=0，说明缓存数组没有内容，因此自然找不到，返回-1
         */
        if (b == -1) {
            return -1;
        }
        /**
         * 如果name比第一个缓存的值都小，说明该缓存中不存在该内容，则直接返回-1
         */
        if (compare(name, array[0].name) < 0) {
            return -1;
        }         
        /**
         * 表示缓存中只有一个值，则因此返回该值下标
         */
        if (b == 0) {
            return 0;
        }
/**
 * 采用二分法查找,如果找到相等的，则直接返回坐标。
 * 如果没有找到相等的。而b-a=1，说明待查找的缓存中经过过滤，就只剩下2个字符了，则让其与最大的字符比较，如果比字符小，则直接返回另外一个字符，如果比该字符大，则返回该字符。
 */
        int i = 0;
        while (true) {
            i = (b + a) / 2;
