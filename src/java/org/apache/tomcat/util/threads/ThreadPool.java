...
public class ThreadPool  {
     ...
     * Default values ...
     */
    public static final int MAX_THREADS = 200;//最多存放的线程数量
    public static final int MAX_THREADS_MIN = 10;
    public static final int MAX_SPARE_THREADS = 50;
    public static final int MIN_SPARE_THREADS = 4;
    public static final int WORK_WAIT_TIMEOUT = 60*1000;//监听器MonitorRunnable间隔时间
     ...
     */
    protected Hashtable threads=new Hashtable();//key是Thread对象，value是获得 的ControlRunnable对象

    protected Vector listeners=new Vector();//监听器，当ControlRunnable被添加到线程池，或者从线程池减少一个的时候，被调用
    ...
    /**
     * Thread priority.设置线程池里面所有的线程的优先级
     */
     ...
     * 1.首先关闭监听器MonitorRunnable对象。
     * 2.循环将活动的线程执行terminate方法。
     * 3.设置currentThreadsBusy和currentThreadCount都为0
     * 
     */
     ...
     * 该方法被监控线程调用，默认呢每小时调用一次。
     * 1.如果该线程池已经停止了，则自然不用调用了。
     * 2.如果当前线程池中线程个数-正在执行的线程数 = 空闲线程数量。该空闲数量大于最大空闲数量时，将会被回收。
     *	基于原则是比如线程池有10个线程，要回收4个。则一定是回收0-3个位置的线程。即忙的线程都在下标大的地方开始。(这部分逻辑需要代码证实) 
     */
     ...
     * 回收线程，忙碌的线程数量减少一个，即currentThreadsBusy减少1个。
     * 存放到则线程池中忙与不忙线程之间存放该回收的线程
     */
     ...
     * throws an exception.
     * 当ControlRunnable的run方法发生异常时，说明ControlRunnable对象不可再用了，因此将currentThreadsBusy和currentThreadCount都减少1个。
     */
