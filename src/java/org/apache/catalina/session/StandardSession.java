...
public class StandardSession
    ...
    public void expire(boolean notify) {
      ...
      //如果已经过期或者已经无效,则直接返回
        ...
        synchronized (this) {
             ...
             */
            long timeNow = System.currentTimeMillis();
            int timeAlive = (int) ((timeNow - creationTime)/1000);//统计session的活跃时间
            synchronized (manager) {
                if (timeAlive > manager.getSessionMaxAliveTime()) {//如果session的活跃时间比最大的session活跃时间还大,则设置最大session活跃时间
                    manager.setSessionMaxAliveTime(timeAlive);
                }
                int numExpired = manager.getExpiredSessions();//统计已经过期的session个数
                if (numExpired < Integer.MAX_VALUE) {
                    numExpired++;//累加
                    manager.setExpiredSessions(numExpired);//重新设置
                }
                ...
                average = (int) (((((long) average) * (numExpired - 1)) + timeAlive)
                        / numExpired);
                manager.setSessionAverageAliveTime(average);//设置平均session活跃时间
            }
     ...
     * 获得该session所在的项目的上下文对象,即属于Context级别的上下文共享对象
     */
     ...
     * check.
     * true表示session可以用,是有效的
     * false表示session无用,是无效的
     */
    ...
    protected void removeAttributeInternal(String name, boolean notify) {
        ...
        Object value = attributes.remove(name);//移除该值
        ...
        if (value instanceof HttpSessionBindingListener) {
            event = new HttpSessionBindingEvent(getSession(), name, value);
            ((HttpSessionBindingListener) value).valueUnbound(event);//发送移除事件
        }
