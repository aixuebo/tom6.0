...
public abstract class ManagerBase implements Manager, MBeanRegistration {
     ...
     * 设置产生sessionid的算法,默认是MD5算法,初始化算法对象
     */
    ...
    /**
     * 存储队列的上限,这个是一个估值,初始化的时候会将下面两个队列的大小填充满,日后再向该队列添加信息的时候,则添加一个的同时,将最上面的元素拿走,即删除
     */
    ...
    // Number of sessions created by this manager创建session的次数
    ...
    //generateSessionId方法生成sessionId的时候,sessionId冲突的次数
    protected int duplicates=0;

    protected boolean initialized=false;//已经初始化完成
     ...
     * 根据设置产生sessionid的算法,默认是MD5算法,设置算法对象
     */
    ...
    protected synchronized String generateSessionId() {
        ...
        do {
            ...
            if (result != null) {
                ...
                duplicates++;//增加sessionId冲突的次数
            }

            while (resultLenBytes < this.sessionIdLength) {//不到sessionId的长度就一直循环
        ...
        } while (sessions.containsKey(result));//如果冲突,则重新循环继续生成sessionId
        return (result);
