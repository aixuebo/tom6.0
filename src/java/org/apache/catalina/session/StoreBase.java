 ...
 * 存储session的基类,目前主要支持针对JDBC和文件的存储
 */
    ...
    public void processExpires() {
        ...
        for (int i = 0; i < keys.length; i++) {
            try {
                ...
                int timeIdle = (int) ((timeNow - session.thisAccessedTime) / 1000L);//获得现在时间与最后一次访问时间之差
                if (timeIdle < session.getMaxInactiveInterval()) {//查看时间是否过期,没过期则continue
                    continue;
                }
                //已经过期
                if (manager.getContainer().getLogger().isDebugEnabled()) {
                ...
                if ( ( (PersistentManagerBase) manager).isLoaded( keys[i] )) {
                    ...
                    session.recycle();//如果已经被存储了,则要设置回收
                } else {
                    // expire swapped out session
                    session.expire();//将该session设置为过期
                }
                remove(keys[i]);//移除该key
            } catch (Exception e) {
