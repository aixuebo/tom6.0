...
public class StandardHost
    ...
    implements Host  
 {
     ...
     * 是否在D:\soft\tomcat6\conf\Catalina\localhost目录下备份一个context.xml文件
     */
     ...
     */
      ...
      * can be detected.
      * 监听器,当子类context执行start之后,要记录属于该host的每一个context的path
      * key是子类的ClassLoader,value是该子类的context的path
      */
      ...
      * 忽略自动部署的项目,即有些项目是需要被忽略的,因此用正则表达式匹配上的都是需要被忽略的
      */
    ...
    public void addAlias(String alias) {
        ...
        synchronized (aliasesLock) {
            // Skip duplicate aliases 查看别名是否存在相同的了
     ...
     * 通过uri找到最接近的Context对象,uri会很长,因此不断缩短该url,按照最长匹配原则,找到最接近的url对应的Context对象
     */
    public Context map(String uri) {
        ...
        String mapuri = uri;
        //uri会很长,因此不断缩短该url,按照最长匹配原则,找到最接近的url对应的Context对象
        while (true) {
