...
public final class StandardServer
    implements Lifecycle, Server, MBeanRegistration 
 {
    ...
    public StandardServer() {
        ...
        //记录全局属性的一些信息,都是字符串信息,key-value形式存储
    ...
    public void addService(Service service) {
        ...
        synchronized (services) {
            ...
            if (started && (service instanceof Lifecycle)) {//已经开始了
                try {
    ...
    public void await() {
        ...
        try {
            awaitSocket =
                new ServerSocket(port, 1,
                                 InetAddress.getByName("localhost"));//开启一个服务端,监听端口
        } catch (IOException e) {
        ...
        try {
            ...
            while (!stopAwait) {//阻塞
                ...
                if (serverSocket == null) {//说明该服务端已经关闭了,因此跳出循环
                    break;
                }
    
                // Wait for the next connection
                Socket socket = null;
                StringBuilder command = new StringBuilder();//接收的命令
                try {
                    InputStream stream = null;
                    try {
                        socket = serverSocket.accept();//接收客户端命令
                ...
                boolean match = command.toString().equals(shutdown);//命令是shutdown
                if (match) {//匹配,因此退出循环
                    break;
                } else
