 ...
 * method itself.
 * 实现应用级别的链式过滤器
 * 当所有的filter都执行完,在调用doFilter的时候,则执行servlet的service方法
...
final class ApplicationFilterChain implements FilterChain, CometFilterChain {
    ...
    public static final int INCREMENT = 10;//每次增长filter队列的长度
    ...
    /**
     * Filters.
     * 一个应用可以有多个filter,因此就有多个fiter的配置信息集合
     */
     ...
     * 对filter设置拦截器,在filter前可以做一些事儿，在filter最后也可以做一些事儿
     */
    ...
    public void doFilter(ServletRequest request, ServletResponse response)
        ...
        if( Globals.IS_SECURITY_ENABLED ) {//是否安全的方式下执行filter
    ...
    //真正执行filter过滤,每执行一次该方法,则选择一个filter进行执行
    private void internalDoFilter(ServletRequest request, 
        ...
        if (pos < n) {
            ApplicationFilterConfig filterConfig = filters[pos++];//执行下一个filter
            Filter filter = null;
            try {
                filter = filterConfig.getFilter();//获取filter对象
                ...
                support.fireInstanceEvent(InstanceEvent.AFTER_FILTER_EVENT,
                                          filter, request, response);//filter之后如何执行
            } catch (IOException e) {
                if (filter != null)
                    support.fireInstanceEvent(InstanceEvent.AFTER_FILTER_EVENT,
        ...
        try {
            ...
            support.fireInstanceEvent(InstanceEvent.BEFORE_SERVICE_EVENT,
                                      servlet, request, response);//service方法前执行
            ...
            support.fireInstanceEvent(InstanceEvent.AFTER_SERVICE_EVENT,
                                      servlet, request, response);//service方法后执行
        } catch (IOException e) {
            support.fireInstanceEvent(InstanceEvent.AFTER_SERVICE_EVENT,
    ...
    private void internalDoFilterEvent(CometEvent event)
        ...
        // Call the next filter if there is one
        if (pos < n) {
            ApplicationFilterConfig filterConfig = filters[pos++]; //获取一个filter
            ...
            try {
                ...
                if( Globals.IS_SECURITY_ENABLED ) {
                    ...
                    filter.doFilterEvent(event, this);//执行该filter事件
                }
