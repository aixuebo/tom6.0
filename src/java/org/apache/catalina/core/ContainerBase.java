...
public abstract class ContainerBase
     ...
     * 存放该容器下所有子容器集合，key是子容器的name，value为Container对象
     */
     ...
     * true表示如果孩子节点被添加进来时，立刻执行该孩子的start方法
     */
    ...
    /**
     * 添加一个容器。
     * 1.判断父容器中是否存在了待添加的容器，如果存在，则抛异常。
     * 2.将子容器的父容器属性设置为父容器类对象。并且将子容器映射在父容器的Map属性中。
     * 3.如果父容器已经开始了，则子容器调用start方法.
     * 4.发送addChild事件
     */
     ...
     *  从管道的第一个Value开始调用invoke方法.如果不存在Value，则调用baseValue的invoke方法
     */
    ...
    public synchronized void addValve(Valve valve) {
        ...
        fireContainerEvent(ADD_VALVE_EVENT, valve);//产生addValve事件
    }
     ...
     * 移除某一个value对象，然后发出removeValve事件
     */
