...
public class Request
     ...
     * 表示已经解析了url和body中的key=value形式的参数
     */
     ...
     * 解析url和body内的key=value形式的paramter参数集合
     */
    protected void parseParameters() {
        ...
        //true表示对url中key=value进行了编码
        boolean useBodyEncodingForURI = connector.getUseBodyEncodingForURI();
        if (enc != null) {
            parameters.setEncoding(enc);
            if (useBodyEncodingForURI) {//设置针对url的编码格式
        ...
        int len = getContentLength();//body中key=value的内容大小

        try {
            if (len > 0) {
                int maxPostSize = connector.getMaxPostSize();//链接最多允许接收字节大小
                if ((maxPostSize > 0) && (len > maxPostSize)) {//如果客户端发出的信息,超出了最多允许的大小,则请求失败
                    if (context.getLogger().isDebugEnabled()) {
                        context.getLogger().debug(sm.getString("coyoteRequest.postTooLarge"));
                    }
                    return;
                }
                byte[] formData = null;//创建缓冲字节数组
     ...
     * 读取body内容,当header头为transfer-encoding:chunked时,body内容解析方式如下:
     */
     ...
     * 解析accept-language的value值
     * value参数内容:zh-CN,zh;q=0.8,en;q=0.6
     */
    protected void parseLocalesHeader(String value) {
      ...
      //key是透明度,例如0.8,value是国家对象List,因为同一个透明度的值可能对应多个国家
        ...
        while (true) {
            ...
            if (start >= length)
                break;
            int end = parser.findChar(',');//找到下一个,号位置
            String entry = parser.extract(start, end).trim();//截取字符串
        ...
        while (keys.hasNext()) {
            Double key = (Double) keys.next();//透明度
