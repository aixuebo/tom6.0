 ...
 * 定义如何解析web.xml格式,并且将解析结果赋值到StandardContext对象中,每一个servlet是一个StandardWrapper对象,StandardWrapper对象被添加到StandardContext对象中
 */
...
final class SetLoginConfig extends Rule {
    ...
    public void begin(String namespace, String name, Attributes attributes)
        throws Exception {
        if (isLoginConfigSet){
            throw new IllegalArgumentException(//login-config标签只能出现一次
...
final class SetJspConfig extends Rule {
    ...
    public void begin(String namespace, String name, Attributes attributes)
        throws Exception {
        if (isJspConfigSet){//检查isJspConfigSet只能被设置一次
...
final class SetSessionConfig extends Rule {
    protected boolean isSessionConfigSet = false;//session-config标签只能出现一次
    public SetSessionConfig() {
    }

    public void begin(String namespace, String name, Attributes attributes)
        throws Exception {
        if (isSessionConfigSet){//session-config标签只能出现一次
            throw new IllegalArgumentException(
