...
public class NamingResources implements Serializable {
     ...
     * 记录全部资源,但是全部资源的name不能重复
     * key为资源的name,value为资源的type属性
     */
     ...
     * key为资源的name,value为ContextEjb对象
     * 对应server.xml中GlobalNamingResources/Ejb标签
     */
     ...
     * name.
     * key为资源的name对象,value为ContextLocalEjb对象
     * 对应server.xml中GlobalNamingResources/LocalEjb标签
     */
    private HashMap localEjbs = new HashMap();

    /**
     * The environment entries for this web application, keyed by name.
     * key为资源的name,value为ContextEnvironment对象
     * 对应解析web.xml中web-app/env-entry标签
     */
     ...
     * keyed by name.
     * key为资源name,value为MessageDestinationRef对象
     * 对应web.xml中解析web.xml的web-app/message-destination-ref标签
     */
     ...
     * keyed by name.
     * key为资源name,value为ContextResourceEnvRef对象
     * 对应server.xml中GlobalNamingResources/ResourceEnvRef标签
     */
     ...
     * key为资源的name,value为ContextResource对象
     * 对应server.xml中GlobalNamingResources/Resource标签
     */
     ...
     * key为资源name,value为ContextResourceLink对象
     * 对应Context/ResourceLink标签,应用于context上下文的标签
     */
     ...
     * key为name,value为ContextService对象
     * 对应server.xml中GlobalNamingResources/ServiceRef标签
     */
     ...
     * 只能有一个ContextTransaction对象在xml中配置
     * 对应server.xml中GlobalNamingResources/Transaction标签
     * 用于存储整个server中共享的key-value信息对
     */
     ...
     * 添加web.xml中解析web.xml的web-app/message-destination-ref标签的内容
     */
