 ...
 */
     ...
     * @param value string
     * 扩展第一个参数数组,将第二个参数添加到第一个参数数组中,组成新的数组返回
     */
     ...
     * @param sArray the StringArray
     * @param value string
     * 判断第二个参数value是否在第一个参数数组中,true表示存在,false表示不存在
     */
    ...
    public void process(Socket theSocket)
        ...
        while (started && !error && keepAlive && !endpoint.isPaused()) {
            ...
            try {
                ...
                inputBuffer.parseRequestLine();//解析第一行
            ...
            try {
                rp.setStage(org.apache.coyote.Constants.STAGE_ENDINPUT);//进入4xx状态码系列
            ...
            try {
                rp.setStage(org.apache.coyote.Constants.STAGE_ENDOUTPUT);//进入5xx状态码系列
    ...
    protected void prepareRequest() {
        ...
        expectation = false;
        if (sslSupport != null) {//如果是安全的,则设置请求头为https
            request.scheme().setString("https");
        }
        
        /**
         * 第一部分，设置http请求协议、http11、http09、keepAlive属性值
         */
        MessageBytes protocolMB = request.protocol();//获得请求协议
        if (protocolMB.equals(Constants.HTTP_11)) {//是否是HTTP/1.1
            http11 = true;
            protocolMB.setString(Constants.HTTP_11);
        } else if (protocolMB.equals(Constants.HTTP_10)) {//如果是HTTP/1.0
            http11 = false;
            keepAlive = false;
            protocolMB.setString(Constants.HTTP_10);
        } else if (protocolMB.equals("")) {//如果没有请求协议，则认为是0.9协议版本
            // HTTP/0.9
            http09 = true;
            http11 = false;
            keepAlive = false;
            ...
            response.setStatus(505);//出异常，则设置为505状态码返回。表示“HTTP版本不受支持”，当服务器不支持请求中所使用的HTTP协议版本时就回返回此错误，最为常见的是 HTTP/1.1。
        }

        /**
         * 第三部分，设置请求方法信息
         */
        MessageBytes methodMB = request.method();//获得请求方法
         ...
         * 第三部分，设置请求头信息
         */
        MimeHeaders headers = request.getMimeHeaders();//获得所有请求头信息

        /**
         * connection请求头中是否包含close,如果包含，则将keepAlive设置为false.
         * 如果包含keep-alive，则加将keepAlive设置为true。
         */
        ...
        ByteChunk uriBC = request.requestURI().getByteChunk();//请求头中uri的？之前的字符串，如果没有?号，则全部uri都是
        if (uriBC.startsWithIgnoreCase("http", 0)) {//是否以http开头的
            ...
            if (pos != -1) {
                ...
                slashPos = uriBC.indexOf('/', pos + 3);//除去http://后第一个出现/的位置
                if (slashPos == -1) {//表示剩余的都是host,没有
                    slashPos = uriBC.getLength();
                    // Set URI as "/"
                    request.requestURI().setBytes(uriB, uriBCStart + pos + 1, 1);//设置uri为/
                } else {//设置uri为/之后的字符，一直到?结束的都是uri
                    request.requestURI().setBytes
                        (uriB, uriBCStart + slashPos,
                         uriBC.getLength() - slashPos);
                }
                MessageBytes hostMB = headers.setValue("host");//设置host为http://之后，/之前的字符
        ...
        // Check host header
        if (http11 && (valueMB == null)) {//http1.1必须有host头
            error = true;
            // 400 - Bad request
            if (log.isDebugEnabled()) {
                log.debug(sm.getString("http11processor.request.prepare")+
 ...
 */
        ...
        if (colonPos < 0) {//不存在端口号
            ...
            request.serverName().setChars(hostNameC, 0, colonPos);//设置:80前的字符到
